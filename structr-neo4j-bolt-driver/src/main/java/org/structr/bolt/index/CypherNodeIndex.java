	...
	protected String getSecurityStatement(QueryContext context, String targetNodeType){
		...
		if (!isAnonymousUser && !isAdmin) {
			...
			buf.append("\nOPTIONAL MATCH (node:NodeInterface")
					...
					.append("WHERE user.id = \"").append(context.getUuid()).append("\"").append(" AND ANY(x IN s.allowed WHERE x = 'read')")
					.append("\n")
					.append("WITH n,result_ContainedGroupOwns+collect(node) AS result_containedGroupsGrant")
					.append("\n")
					.append("UNWIND result_containedGroupsGrant as visibleNode")
					.append("\n")
					.append("OPTIONAL MATCH path=(startSchemaNode:NodeInterface:SchemaNode)-[]-(:SchemaRelationshipNode)-[*1..5]-(:SchemaRelationshipNode)-[]-(endSchemaNode:NodeInterface:SchemaNode)")
					.append("\n")
					.append("WHERE startSchemaNode.name IN labels(visibleNode) AND ALL(x in FILTER(y IN NODES(path) WHERE y.type=\"SchemaRelationshipNode\") WHERE \"Keep\" IN x.readPropagation OR \"Add\" IN x.readPropagation)")
					.append("\n")
					.append("WITH n,EXTRACT(x IN FILTER(y IN NODES(path) WHERE y.type=\"SchemaNode\") | x.name) as pathTypes,endSchemaNode, visibleNode, result_containedGroupsGrant")
					.append("\n")
					.append("OPTIONAL MATCH path=(visibleNode)-[*1..5]-(endNode)")
					.append("\n")
					.append("WHERE EXTRACT(x IN NODES(path) | x.type) = pathTypes AND endNode.type = endSchemaNode.name")
					.append("\n")
					.append("WITH n,FILTER(x IN NODES(path) WHERE x<>visibleNode) as nodesVisibleThroughPropagation, result_containedGroupsGrant")
					.append("\n")
					.append("WITH n,result_containedGroupsGrant+collect(nodesVisibleThroughPropagation) as totalResult")
					.append("\n")
